...
func (t *Tracker) processLostEvents() {
	...
	for {
		...
		// Since this is an end-state goroutine, it should be terminated only when Tracker done channel is closed.
		case <-t.done:
			return
		}
	}
...
// registerEventProcessors registers tracker's internal default event processors
...
func (t *Tracker) convertArgMonotonicToEpochTime(event *trace.Event, argName string) error {
...
// was loaded and tracker needs to check if it hooked the syscall table and
// seq_ops
