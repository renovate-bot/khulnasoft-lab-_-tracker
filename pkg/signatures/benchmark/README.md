...
The `tracker-rules` binary implements pluggable heuristics to detect suspicious behavioral patterns based on events
collected by the `tracker-ebpf` binary. By default, `tracker-ebpf` binary prints events to the standard output, which is
redirected to the standard input of the `tracker-rules` binary:

```console
tracker-ebpf --output=format:gob --output=option:parse-arguments --events mem_prot_alert | tracker-rules --input-tracker=file:stdin --input-tracker=format:gob
```

The heuristics implemented by `tracker-rules` are either Go or Rego signatures loaded on startup as Go plugins or Rego
scripts. Input events captured by `tracker-ebpf` are read from the standard input, decoded, and sent on the Go input
...
- [ ] Find out how `tracker-rules` scales with the number of signatures.
...
Apparently `tracker-rules` does not scale well with the number of signatures. With increasing number of signatures
