...
```console
sudo ./dist/tracker --scope help
sudo ./dist/tracker --events help
sudo ./dist/tracker --scope xxx --events xxx
```

Tracker output might become too hard to consume when tracing all the events from
a system. Luckily, Tracker has a powerful mechanism to accurately filter just the
information that is relevant to the user using the `--scope` and `--events` flags.

With those command line flags you define expressions that tell **tracker**
...
!!! Tip
    You can filter events by most of the visible fields from Tracker events.

## Initial Example

All the examples bellow this item can be executed with the following tracker
prefix command:
...
1. **Event** and **Scope** `(Operators: event "-" and scope "follow". Prefix/Suffix: *)`
    ...
    !!! Note
        The "follow" operator will make tracker follow all newly created
...
1. **Event Arguments** `(Operators: =, !=. Prefix/Suffix: *)`
    ...
    !!! Tip
        ...
        since by `--events openat.args.pathname=/etc/shadow` tracker infers that openat must be
        filtered.
...
1. **Event Sets**
    ...
    !!! Note
        Selects a set of events to tracker according to pre-defined sets which
...
1. **PID** `(Operators: =, !=, <, > and "new")`
    ...
    ```text
    1) --scope pid=new # newly created events (after tracker execution)
