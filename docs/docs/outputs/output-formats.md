...
<!-- TODO: build man page -->
The `--output` flag controls where and how Tracker will output events, by specifying `--output <format>:<destination>`.  You can use the `--output` flag multiple times to output events in multiple ways.<!-- To see all output options you can run `man tracker-output`. -->
...
```console
sudo ./dist/tracker --output table --scope comm=bash --scope follow --events openat
```
...
### Table (Verbose)
    ...
    ```console
    sudo ./dist/tracker --output table-verbose --scope comm=bash --scope follow --events openat
    ```
...
```console
sudo ./dist/tracker --output json --scope comm=bash --scope follow --events openat
```
...
!!! Tip
    A good tip is to pipe **tracker** json output to [jq]() tool, this way
    you can select fields, rename them, filter values, and many other things:
    > ```console
    > sudo ./dist/tracker -o json -o option:parse-arguments
